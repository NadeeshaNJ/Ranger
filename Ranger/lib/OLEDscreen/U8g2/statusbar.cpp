#include <Arduino.h>
#include <Wire.h>
#include <U8g2lib.h>

#define SCREEN_WIDTH 128 // OLED display width, in pixels
#define SCREEN_HEIGHT 64 // OLED display height, in pixels

U8G2_SSD1306_128X64_NONAME_F_HW_I2C u8g2(U8G2_R0, U8X8_PIN_NONE);

// [BEGIN lopaka generated]
static const unsigned char image_battery_bits[] U8X8_PROGMEM = {0xff,0x07,0x01,0x04,0x01,0x0c,0x01,0x04,0xff,0x07};
static const unsigned char image_gps_symbol_bits[] U8X8_PROGMEM = {0x02,0x05,0x1a,0x1c,0x2c,0x50,0x20};
static const unsigned char image_gps_bar1_bits[] U8X8_PROGMEM = {0x03,0x03};
static const unsigned char image_gps_bar2_bits[] U8X8_PROGMEM = {0x03,0x03,0x03,0x03};
static const unsigned char image_gps_bar3_bits[] U8X8_PROGMEM = {0x03,0x03,0x03,0x03,0x03,0x03};


void drawStatus(int gpsSignal = 0, int BatteryLevel = 0.5, const char* time = "14:35") {
    u8g2.setFontMode(1);
    u8g2.setBitmapMode(1);
    // status_line
    u8g2.drawLine(0, 8, 127, 8);
    // time
    u8g2.setFont(u8g2_font_haxrcorp4089_tr);
    u8g2.drawStr(53, 7, time);
    /*
    ------------------------------------
    Battery levels
    ------------------------------------
    // battery_10%
    u8g2.drawFrame(116, 2, 1, 3);
    // battery_25%
    u8g2.drawBox(116, 2, 3, 3);
    // battery_50%
    u8g2.drawBox(116, 2, 5, 3);
    // battery_75%
    u8g2.drawBox(116, 2, 7, 3);
    // battery_100%
    u8g2.drawBox(116, 2, 9, 3);
    ------------------------------------
    */    
    u8g2.drawXBMP(115, 1, 12, 5, image_battery_bits); // battery icon
    if (BatteryLevel >= 0.05) u8g2.drawFrame(116, 2, 1, 3);
    if (BatteryLevel >= 0.15) u8g2.drawBox(116, 2, 3, 3);
    if (BatteryLevel >= 0.4) u8g2.drawBox(116, 2, 5, 3);
    if (BatteryLevel >= 0.65) u8g2.drawBox(116, 2, 7, 3);
    if (BatteryLevel >= 0.9) u8g2.drawBox(116, 2, 9, 3);
    /*
    -------------------------------------
    GPS symbol and signal bars
    -------------------------------------
    // gps_symbol
    u8g2.drawXBMP(7, 0, 7, 7, image_gps_symbol_bits);
    // gps_bar1
    u8g2.drawXBMP(6, 5, 2, 2, image_gps_bar1_bits);
    // gps_bar2
    u8g2.drawXBMP(3, 3, 2, 4, image_gps_bar2_bits);
    // gps_bar3
    u8g2.drawXBMP(0, 1, 2, 6, image_gps_bar3_bits);
    */
    if (gpsSignal > 0) u8g2.drawXBMP(7, 0, 7, 7, image_gps_symbol_bits);
    if (gpsSignal >= 1) u8g2.drawXBMP(6, 5, 2, 2, image_gps_bar1_bits);
    if (gpsSignal >= 2) u8g2.drawXBMP(3, 3, 2, 4, image_gps_bar2_bits);
    if (gpsSignal >= 3) u8g2.drawXBMP(0, 1, 2, 6, image_gps_bar3_bits);
}

void loop(){
    u8g2.firstPage();
    do {
        drawStatus();
    } while (u8g2.nextPage());
};
